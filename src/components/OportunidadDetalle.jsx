...
const getEtapaLabel = (etapa) => ETAPAS_CLIENTE[etapa]?.label || (etapa || 'oportunidad_nuevo');
...
export default function OportunidadDetalle({
    Oportunidad: initialOportunidad,
    rolePath,
    onVolver,
    onActualizado,
    abrirModalEditar
    ...
    const [OportunidadSeleccionado, setOportunidadSeleccionado] = useState(initialOportunidad);
    const pid = OportunidadSeleccionado?.id || OportunidadSeleccionado?._id;

    const [actividadesContext, setActividadesContext] = useState([]);
    const [loadingContext, setLoadingContext] = useState(false);

    const [notasRapidas, setNotasRapidas] = useState(initialOportunidad?.notas || '');
    ...
    const [monedaSeleccionada, setMonedaSeleccionada] = useState(initialOportunidad?.customMetricLabel || 'MXN');
    const [valorOportunidad, setValorOportunidad] = useState(initialOportunidad?.customMetricValue || '');
    ...
    const [customSections, setCustomSections] = useState(() =>
        parseOrDefaultSections(initialOportunidad?.customSections)
    );
    ...
    useEffect(() => {
        if (initialOportunidad) {
            // Solo sobreescribir si no estamos en medio de una edición o si el ID cambió realmente
            const currentId = OportunidadSeleccionado?.id || OportunidadSeleccionado?._id;
            const newId = initialOportunidad.id || initialOportunidad._id;

            if (currentId !== newId) {
                setOportunidadSeleccionado(initialOportunidad);
                setNotasRapidas(initialOportunidad.notas || '');
                setMonedaSeleccionada(initialOportunidad.customMetricLabel || 'MXN');
                setValorOportunidad(initialOportunidad.customMetricValue || '');

                // Parsear customSections si viene como string
                setCustomSections(parseOrDefaultSections(initialOportunidad.customSections));
            }
        }
    }, [initialOportunidad]);

    // Solo cargar el historial cuando cambie el ID del Oportunidad
    useEffect(() => {
        if (initialOportunidad && (initialOportunidad.id || initialOportunidad._id)) {
            handleSeleccionarOportunidadProp(initialOportunidad);
            cargarRecordatorios(initialOportunidad.id || initialOportunidad._id);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [initialOportunidad?.id, initialOportunidad?._id]);

    const cargarRecordatorios = async (OportunidadId) => {
        try {
            const res = await axios.get(`${API_URL}/api/${rolePath}/prospectos/${OportunidadId}/recordatorios`, { headers: getAuthHeaders() });
    ...
    const handleSeleccionarOportunidadProp = async (p) => {
        ...
        try {
            ...
            console.error(error);
            toast.error('Error al cargar historial del Oportunidad');
    ...
    const handleSeleccionarOportunidad = () => {
        handleSeleccionarOportunidadProp(OportunidadSeleccionado);
    };
    ...
    const actualizarInteres = async (id, nuevoInteres) => {
        try {
            ...
            setOportunidadSeleccionado({ ...OportunidadSeleccionado, interes: nuevoInteres });
    ...
    const handleGuardarNotasRapidas = async () => {
        if (!OportunidadSeleccionado) return;
        setLoadingNotas(true);
        try {
            const pidLoc = OportunidadSeleccionado.id || OportunidadSeleccionado._id;
            await axios.put(`${API_URL}/api/${rolePath}/prospectos/${pidLoc}/editar`, {
                titulo: OportunidadSeleccionado.titulo || '',
                apellidoPaterno: OportunidadSeleccionado.apellidoPaterno || '',
                apellidoMaterno: OportunidadSeleccionado.apellidoMaterno || '',
                telefono: OportunidadSeleccionado.telefono || '',
                telefono2: OportunidadSeleccionado.telefono2 || '',
                correo: OportunidadSeleccionado.correo || '',
                empresa: OportunidadSeleccionado.empresa || '',
                sitioWeb: OportunidadSeleccionado.sitioWeb || '',
                ubicacion: OportunidadSeleccionado.ubicacion || '',
                notas: notasRapidas
            }, { headers: getAuthHeaders() });

            toast.success('Notas guardadas');
            setOportunidadSeleccionado(prev => ({ ...prev, notas: notasRapidas }));
            if (onActualizado) onActualizado();
        } catch (error) {
            console.error(error);
    ...
    const handleGuardarMetricaPersonalizada = async () => {
        ...
        try {
            ...
            await axios.put(`${API_URL}/api/${rolePath}/prospectos/${pidLoc}`, {
                ...
                customMetricValue: valorOportunidad
            }, { headers: getAuthHeaders() });
            setOportunidadSeleccionado(prev => ({ ...prev, customMetricLabel: monedaSeleccionada, customMetricValue: valorOportunidad }));
            if (onActualizado) onActualizado();
        } catch (error) {
    ...
    const handleGuardarSeccionesPersonalizadas = async (nuevasSecciones) => {
        ...
        try {
            ...
            setOportunidadSeleccionado(prev => ({ ...prev, customSections: nuevasSecciones }));
            if (onActualizado) onActualizado();
        } catch (error) {
    ...
    const getActIcon = (act) => {
        ...
        if (act.tipo === 'oportunidad') return { icon: '🏆', color: 'bg-yellow-500', label: 'Convertido a oportunidad' };
    ...
    const registrarActividad = async (payload) => {
        ...
        try {
            ...
            if (
                ...
                OportunidadSeleccionado.etapaEmbudo === 'Oportunidad_nuevo'
            ) {
            ...
            if (payload.tipo === 'llamada' && OportunidadSeleccionado.proximaLlamada) {
                await axios.put(`${API_URL}/api/${rolePath}/prospectos/${pid}`, {
                    proximaLlamada: null
                }, { headers: getAuthHeaders() });
            }

            await axios.post(`${API_URL}/api/${rolePath}/registrar-actividad`, { oportunidadId: pid, ...payloadFinal }, { headers: getAuthHeaders() });
            toast.success('Interacción registrada');

            // Recargar Oportunidad fresco desde el servidor (evitar estado obsoleto)
            ...
            if (updated) {
                ...
                setOportunidadSeleccionado(prev => ({ ...updated, customSections: localSections }));
                setOportunidads(res.data);
            }
            // Recargar historial de actividades sin reemplazar customSections
            handleSeleccionarOportunidad(updated || OportunidadSeleccionado, { preserveCustomSections: true });
            return true;
        } catch {
    ...
    const handleMarcarCitaRealizada = async (cita) => {
        ...
        try {
            ...
            if (!quedanPendientes && OportunidadSeleccionado.etapaEmbudo === 'reunion_agendada') {
                await axios.put(`${API_URL}/api/${rolePath}/prospectos/${pid}/editar`, {
                    ...
                    sitioWeb: OportunidadSeleccionado.sitioWeb || '',
                    ubicacion: OportunidadSeleccionado.ubicacion || '',
                    notas: OportunidadSeleccionado.notas || '',
                    etapaEmbudo: 'reunion_realizada'
                }, { headers: getAuthHeaders() });
                setOportunidadSeleccionado(prev => ({ ...prev, etapaEmbudo: 'reunion_realizada' }));
            }

            toast.success('Cita marcada como realizada');
            if (onActualizado) onActualizado();
            handleSeleccionarOportunidad(OportunidadSeleccionado);
        } catch (error) {
            console.error(error);
    ...
    const handleCambiarEtapa = async (nuevaEtapa) => {
        ...
        try {
            await axios.put(`${API_URL}/api/${rolePath}/prospectos/${OportunidadSeleccionado.id || OportunidadSeleccionado._id}/editar`,
                {
                    etapaOportunidad: nuevaEtapa
                },
                { headers: getAuthHeaders() }
            );
            setOportunidadSeleccionado(prev => ({ ...prev, etapaOportunidad: nuevaEtapa }));
    ...
    const handleEtiquetasChange = async (nuevasEtiquetas) => {
        try {
            ...
            setOportunidadSeleccionado(prev => ({ ...prev, etiquetas: etiquetasStr }));
            if (onActualizado) onActualizado();
        } catch (error) {
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-4 overflow-y-auto hide-scrollbar pr-1">
                        ...
                        <div className="bg-white border border-slate-200 rounded-xl p-4 sm:px-6 shadow-sm shrink-0 relative overflow-hidden">
                            <div className="flex flex-col gap-3">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <div className="flex items-center gap-3 flex-wrap">
                                            <h1 className="text-2xl font-bold text-gray-900 leading-tight">
                                                {OportunidadSeleccionado.titulo} {OportunidadSeleccionado.apellidoPaterno}
                                            </h1>
                                            <button
                                                onClick={() => abrirModalEditar(OportunidadSeleccionado)}
                                                className="p-1.5 text-slate-400 hover:text-(--theme-600) hover:bg-(--theme-50) rounded-full transition-all"
                                                title="Editar información del Oportunidad"
                                            >
                                        ...
                                        <div className="flex items-center gap-3 flex-wrap">
                                            {editandoEtapa ? (
                                                <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-md p-0.5 shadow-sm">
                                                    <select
                                                        ...
                                                        defaultValue={OportunidadSeleccionado.etapaOportunidad || 'oportunidad_nuevo'}
                                                ...
                                                <div className="flex items-center gap-2 group/etapa cursor-pointer">
                                                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-medium tracking-wide uppercase shadow-sm ${getEtapaColor(OportunidadSeleccionado.etapaOportunidad || 'oportunidad_nuevo')}`}>
                                                        {getEtapaLabel(OportunidadSeleccionado.etapaOportunidad || 'oportunidad_nuevo')}
                                                    </span>
                                                    <button
                                            ...
                                            <div className="ml-2 pl-2 border-l border-gray-200">
                                                <GestorEtiquetas 
                                                    oportunidadEtiquetas={OportunidadSeleccionado.etiquetas}
                                                    ...
                                                    <div className="flex items-center gap-0.5 text-yellow-500">
                                                        {[1, 2, 3, 4, 5].map((value) => (
                                                            <button
                                                                key={value}
                                                                type="button"
                                                                onClick={() => actualizarInteres(OportunidadSeleccionado.id || OportunidadSeleccionado._id, OportunidadSeleccionado.interes === value ? 0 : value)}
                                                                className="hover:scale-110 transition-transform active:scale-95 px-0.5"
                                                                title={`Nivel de interés: ${value} de 5`}
                                                            >
                                                                <Star className={`w-5.5 h-5.5 ${OportunidadSeleccionado.interes >= value ? 'fill-yellow-400' : 'fill-slate-100 text-slate-300'}`} />
                                                            </button>
                                                        ))}
                                                    </div>
                                                </div>
                                            </div>
                                            {OportunidadSeleccionado.empresa && (
                                ...
                                {(OportunidadSeleccionado.telefono || OportunidadSeleccionado.correo || OportunidadSeleccionado.ubicacion || OportunidadSeleccionado.sitioWeb) && (
                                    <div className="pt-2">
                                        ...
                                        <div className="hidden lg:flex flex-wrap items-center gap-x-4 gap-y-2 pt-3 border-t border-slate-100">
                                            {/* Teléfonos */}
                                            {(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2) && (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="flex flex-col overflow-hidden">
                                                        ...
                                                        <div className="flex flex-wrap text-xs font-bold text-slate-700 truncate">
                                                            {[OportunidadSeleccionado.telefono, OportunidadSeleccionado.telefono2].filter(Boolean).flatMap(t => t.split(',').map(s => s.trim())).filter(Boolean).slice(0, 1).map((tel, idx) => (<span key={idx} className="truncate">{tel}</span>))}
                                                            {[OportunidadSeleccionado.telefono, OportunidadSeleccionado.telefono2].filter(Boolean).flatMap(t => t.split(',').map(s => s.trim())).filter(Boolean).length > 1 && (<span className="ml-1 text-slate-400 text-[10px]">...</span>)}
                                                        </div>
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            {/* Correo */}
                                            {OportunidadSeleccionado.correo && (
                                                <div className="flex items-center gap-2">
                                                    <div className="w-7 h-7 flex items-center justify-center bg-slate-50 rounded-lg text-slate-400 shrink-0">
                                                    ...
                                                    <div className="flex flex-col overflow-hidden">
                                                        ...
                                                        <span className="text-xs font-bold text-slate-700 truncate" title={OportunidadSeleccionado.correo}>
                                                            {OportunidadSeleccionado.correo}
                                                        </span>
                                                    </div>
                                                </div>
                                            ...
                                            {/* Ubicación */}
                                            {OportunidadSeleccionado.ubicacion && (
                                                <div className="flex items-center gap-2">
                                                    <div className="w-7 h-7 flex items-center justify-center bg-slate-50 rounded-lg text-slate-400 shrink-0">
                                            ...
                                            {/* Sitio Web */}
                                            {OportunidadSeleccionado.sitioWeb && (
                                                <div className="flex items-center gap-2">
                                                    <div className="w-7 h-7 flex items-center justify-center bg-slate-50 rounded-lg text-slate-400 shrink-0">
                                                    ...
                                                    <div className="min-w-0 flex flex-col overflow-hidden">
                                                        ...
                                                        <a
                                                            href={OportunidadSeleccionado.sitioWeb.startsWith('http') ? OportunidadSeleccionado.sitioWeb : `https://${OportunidadSeleccionado.sitioWeb}`}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="text-xs font-bold text-(--theme-600) hover:underline truncate"
                                                        >
                                                            {OportunidadSeleccionado.sitioWeb.replace(/^https?:\/\//, '')}
                                                        </a>
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            <div className="ml-auto flex items-center gap-2 shrink-0">
                                                <PlantillasMensajesModal contacto={OportunidadSeleccionado} scope="oportunidad" />
                                                <a
                                                    href={(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2) ? `https://wa.me/${[OportunidadSeleccionado.telefono, OportunidadSeleccionado.telefono2].filter(Boolean).join(',').replace(/\D/g, '')}` : undefined}
                                                    target={(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2) ? '_blank' : undefined}
                                                    rel={(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2) ? 'noopener noreferrer' : undefined}
                                                    aria-disabled={!(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2)}
                                                    onClick={!(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2) ? (e) => e.preventDefault() : undefined}
                                                    className={`h-8 w-8 inline-flex items-center justify-center rounded-lg transition-all ${(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2) ? 'bg-green-50 text-green-600 hover:bg-green-100 hover:scale-105' : 'bg-slate-100 text-slate-400 cursor-not-allowed opacity-60'}`}
                                                    title={(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2) ? 'Mensaje por WhatsApp' : 'No hay teléfono para WhatsApp'}
                                                >
                                                ...
                                                <a
                                                    href={OportunidadSeleccionado.correo ? `mailto:${OportunidadSeleccionado.correo}` : undefined}
                                                    aria-disabled={!OportunidadSeleccionado.correo}
                                                    onClick={!OportunidadSeleccionado.correo ? (e) => e.preventDefault() : undefined}
                                                    className={`h-8 w-8 inline-flex items-center justify-center rounded-lg transition-all ${OportunidadSeleccionado.correo ? 'bg-slate-50 hover:bg-slate-100 hover:scale-105' : 'bg-slate-100 cursor-not-allowed opacity-60'}`}
                                                    title={OportunidadSeleccionado.correo ? 'Enviar correo por Gmail' : 'No hay correo para Gmail'}
                                                >
                                                    <img src={GmailIcon} alt="Gmail" className={`w-4 h-4 object-contain ${OportunidadSeleccionado.correo ? '' : 'grayscale opacity-60'}`} />
                                                </a>
                                            </div>
                                        </div>
                                        ...
                                        <div className="lg:hidden mt-3">
                                                ...
                                                {OportunidadSeleccionado.ubicacion && (
                                                    <div className="flex items-start gap-2.5 p-2 rounded-lg bg-slate-50 border border-slate-100">
                                                        ...
                                                        <div className="flex flex-col min-w-0">
                                                            ...
                                                            <span className="text-xs font-bold text-slate-700 truncate" title={OportunidadSeleccionado.ubicacion}>
                                                                {OportunidadSeleccionado.ubicacion}
                                                            </span>
                                                        </div>
                                                    </div>
                                            ...
                                            <div className="flex items-center justify-end gap-2.5 mt-2 pt-2 border-t border-slate-100">
                                                ...
                                                <a
                                                    ...
                                                    aria-disabled={!(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2)}
                                                    onClick={!(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2) ? (e) => e.preventDefault() : undefined}
                                                    className={`h-10 w-10 inline-flex items-center justify-center rounded-lg transition-all shadow-sm border ${(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2) ? 'bg-green-50 hover:bg-green-100 border-green-200 hover:scale-105' : 'bg-slate-100 border-slate-200 cursor-not-allowed opacity-60'}`}
                                                    title={(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2) ? 'Mensaje por WhatsApp' : 'No hay teléfono para WhatsApp'}
                                                >
                                                    <svg viewBox="0 0 24 24" className={`w-5 h-5 ${(OportunidadSeleccionado.telefono || OportunidadSeleccionado.telefono2) ? 'fill-green-600' : 'fill-slate-400'}`}><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" /></svg>
                                                </a>
                                                <a
                                                    href={OportunidadSeleccionado.correo ? `mailto:${OportunidadSeleccionado.correo}` : undefined}
                                                    aria-disabled={!OportunidadSeleccionado.correo}
                                                    onClick={!OportunidadSeleccionado.correo ? (e) => e.preventDefault() : undefined}
                                                    className={`h-10 w-10 inline-flex items-center justify-center rounded-lg transition-all shadow-sm border ${OportunidadSeleccionado.correo ? 'bg-slate-50 hover:bg-slate-100 border-slate-200 hover:scale-105' : 'bg-slate-100 border-slate-200 cursor-not-allowed opacity-60'}`}
                                                    title={OportunidadSeleccionado.correo ? 'Enviar correo por Gmail' : 'No hay correo para Gmail'}
                                                >
                                                    <img src={GmailIcon} alt="Gmail" className={`w-5 h-5 object-contain ${OportunidadSeleccionado.correo ? '' : 'grayscale opacity-60'}`} />
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        {/* ==================== KPIs ROTATIVAS ==================== */}
                        <KpiRotativas
                            OportunidadSeleccionado={OportunidadSeleccionado}
                            actividadesContext={actividadesContext}
                            llamadasExitosas={llamadasExitosas}
                            llamadasFallidas={llamadasFallidas}
                            valorOportunidad={valorOportunidad}
                            setValorOportunidad={setValorOportunidad}
                        ...
                        <div className="space-y-3">
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                ...
                                {/* Agendar reunión */}
                                <button
                                    onClick={() => navigate(`/${calendarRolePath}/calendario`, { state: { prospecto: OportunidadSeleccionado, Oportunidad: OportunidadSeleccionado, oportunidad: OportunidadSeleccionado } })}
                                    className="flex flex-col items-center justify-center gap-2 bg-white border-2 border-slate-200 hover:border-(--theme-500) rounded-xl p-4 text-gray-700 hover:text-(--theme-600) transition-all shadow-sm font-bold text-sm text-center leading-tight"
                                >
                                ...
                                <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex flex-col h-[240px]">
                                    <div className="flex items-center justify-between shrink-0">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Notas del Oportunidad</p>
                                        </div>
                                        <button
                                            onClick={handleGuardarNotasRapidas}
                                            disabled={loadingNotas}
                                            className={`p-1.5 rounded-lg transition-colors ${notasRapidas !== (OportunidadSeleccionado?.notas || '') ? 'bg-(--theme-500) text-white hover:bg-(--theme-600) shadow-sm' : 'text-slate-300 hover:bg-slate-50'}`}
                                            title="Guardar notas"
                                        >
                                                    ...
                                                    act.tipo === 'cita' ? 'bg-(--theme-500)' :
                                                        act.tipo === 'llamada' && act.resultado === 'fallido' ? 'bg-rose-400' :
                                                            act.tipo === 'llamada' ? 'bg-(--theme-500)' :
                                                                act.tipo === 'oportunidad' ? 'bg-yellow-500' :
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <button
                                            onClick={async () => {
                                                ...
                                                if (!ok) return;
                                                setLlamadaFlow(null);
                                                navigate(`/${calendarRolePath}/calendario`, { state: { prospecto: OportunidadSeleccionado, Oportunidad: OportunidadSeleccionado, oportunidad: OportunidadSeleccionado } });
                                            }}
                                            disabled={estaBloqueadoRegistro}
                                        ...
                                        <button
                                            onClick={async () => {
                                                try {
                                                    ...
                                                    setRegistrandoActividad(true);
                                                    const pidLocal = OportunidadSeleccionado.id || OportunidadSeleccionado._id;
                                                    ...
                                                    if (updated) { setOportunidadSeleccionado(updated); setOportunidads(res.data); }
                            ...
                            {llamadaFlow.paso === 'otro' && (
                                <div className="space-y-3">
                                    ...
                                    <textarea
                                        ...
                                        value={llamadaFlow.notas || ''}
                                        onChange={e => setLlamadaFlow(f => ({ ...f, notas: e.target.value }))}
                                        placeholder="Ej: Oportunidad preguntará de presupuesto luego, prometió repasar propuesta..."
                                    ...
                                    <button
                                        onClick={async () => {
                                            try {
                                                ...
                                                if (llamadaFlow.fechaProxima) {
                                                    // 2. Actualizar solo proximaLlamada (ruta simple, no requiere titulo/telefono)
                                                    await axios.put(`${API_URL}/api/${rolePath}/prospectos/${pidLocal}`, {
                                                        proximaLlamada: toUtcIsoFromLocalInput(llamadaFlow.fechaProxima)
                                                    }, { headers: getAuthHeaders() });
                                                }
                        ...
                        <div className="px-6 py-5 border-b border-slate-100 bg-linear-to-r from-(--theme-50) to-white">
                            <div className="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p className="text-sm text-slate-600">Configura una fecha rápida para volver a llamar a {OportunidadSeleccionado.titulo}.</p>
                                </div>
                                <button
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex items-center gap-3 p-3 bg-(--theme-50) border border-(--theme-100) rounded-xl">
                                        ...
                                        <div className="text-xs">
                                            <p className="font-bold text-(--theme-700)">Reunión con {OportunidadSeleccionado.titulo}</p>
                                            <p className="text-(--theme-600)">Empresa: {OportunidadSeleccionado.empresa || 'No especificada'}</p>
                                        </div>
                                    </div>
                                </div>
                            )}
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
                        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                            <div>
                                <h2 className="text-base font-bold text-gray-900">Añadir Módulo al Oportunidad</h2>
